...
namespace Module\Admins\Http\Controllers\Web\Panel;
...
use Module\Admins\Http\Resources\AdminResource;
use Module\Admins\Http\Resources\AdminCollection;
...
class AdminsController extends Controller
{
    ...
    {
        if(request()->ajax()){
            $cacheName = Auth::user()->id._settings('settings', 'system_key').'-adminTable';
            return cache()->remember($cacheName, _settings('general', 'cache_remember_time'), function(){
                $query = Admin::with('adminStatus')->with('adminTitle')->with('adminGender')->whereNotIn('id', ['1', Auth::user()->id])->get();
